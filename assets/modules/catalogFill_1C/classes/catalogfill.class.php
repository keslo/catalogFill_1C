 ...
 * @param int $parent_id
 */
function csv_import($parent_id,$file_name=false,$start1Csync){

    //очистка категории
    if($this->config['imp_chk_field']===false && $this->config['imp_chk_tvid_val']===false){
    ...
    if($file_uploaded || isset($start1Csync)){
        $count = 0;
...
/**
 * Сохранение настроек синхронизации с 1С
 * 
 *
 * @return string
 */

function save_settings_1C_sync($link_settings,$file_settings,$parent_id_settings){
    $file = fopen($this->config['files_settings_dir']."settings.conf.php","w"); 
    $text="<?php
        \$settings=array(
        'link_settings'=>'".$link_settings."',
        'file_settings'=>'".$file_settings."',
        'parent_id_settings'=>'".$parent_id_settings."'
        );";
    fwrite($file, $text);
    fclose($file);

    if($file){$this->message = 'Настройки успешно сохранены';}else{$this->message = 'Произошла ошибка при открытии файла настроек';}
}

/**
 * Извлечение настроек синхронизации с 1С
 * 
 *
 * @return string
 */

function getSettings1Csync($case){
    require $this->config['files_settings_dir']."settings.conf.php";
    switch ($case){
        case "link_settings": echo $settings['link_settings'];
        break;
        case "file_settings": echo $settings['file_settings'];
        break;
        case "parent_id_settings": echo $settings['parent_id_settings'];
        break;
    }
}
